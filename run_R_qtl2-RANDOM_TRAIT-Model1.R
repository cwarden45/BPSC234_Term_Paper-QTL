...
output.file1 = paste(cross,"_RANDOM_TRAIT.Model1_hist.png",sep="")
output.file2 = paste(cross,"_RANDOM_TRAIT.Model1_LODpeaks.txt",sep="")
output.file3 = paste(cross,"_RANDOM_TRAIT.Model1_LODbayes.txt",sep="")
output.file4 = paste(cross,"_RANDOM_TRAIT.Model1_LODeffect.txt",sep="")
output.plot = paste(cross,"_RANDOM_TRAIT.Model1_LODall.png",sep="")
...
phenoOUT = paste(cross,"_pheno_RANDOM_TRAIT.csv",sep="")
...
set.seed(0)
png(output.file1, type="cairo")
par(mfcol=c(1,2))
sample_mean = mean(QTL2_pheno.table[,pheno_to_test])
print(sample_mean)
sample_sd = sd(QTL2_pheno.table[,pheno_to_test])
print(sample_sd)
hist(QTL2_pheno.table[,pheno_to_test], main = pheno_to_test, col="gray")
RANDOM_PHENO = rnorm(n=nrow(QTL2_pheno.table), mean=sample_mean, sd=sample_sd)
hist(RANDOM_PHENO, main = "Random Phenotype", col="gray")
dev.off()
QTL2_pheno.table = data.frame(id = QTL2_pheno.table$id, RANDOM_PHENO)
...
bayes = bayes_int(out, map, chr = "chrXIV", lodcolumn=1, prob=0.95)
